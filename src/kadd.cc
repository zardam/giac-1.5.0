...
void mastermind_disp(const vector<int> & solution,const vector< vector<int> > & essais,const vector<int> & essai,bool fulldisp,GIAC_CONTEXT){
  ...
  if (fulldisp){
	...
	if (S[s]<E[e])
      ...
      char buf[2]={0,0};
      buf[0]='0'+bien;
      numworks_draw_string(x0+20*c+3,y0+20*4+2,COLOR_GREEN,_WHITE,buf);
      buf[0]='0'+mal;
      numworks_draw_string(x0+20*c+3,y0+20*5+2,COLOR_MAGENTA,_WHITE,buf);
      //CERR << solution << " " << essai << " " << bien << " " << mal << endl;
    }
  }
  int y=170;
  int x=numworks_draw_string_small(x0,y,"0");
  draw_filled_circle(x+10,y+10,10,COLOR_BLUE);
  x=numworks_draw_string_small(x+30,y,"1");
  draw_filled_circle(x+10,y+10,10,COLOR_RED);
  x=numworks_draw_string_small(x+30,y,"2");
  draw_filled_circle(x+10,y+10,10,COLOR_MAGENTA);
  x=numworks_draw_string_small(x+30,y,"3");
  draw_filled_circle(x+10,y+10,10,COLOR_GREEN);
  x=numworks_draw_string_small(x+30,y,"4");
  draw_filled_circle(x+10,y+10,10,COLOR_CYAN);
  x=numworks_draw_string_small(x+30,y,"5");
...
int fractale(GIAC_CONTEXT){
  ...
  for (int y=0;y<=Y/2;++y){
	...
	if (abs(z)>2)
      ...
      numworks_set_pixel(x,y,color);
      numworks_set_pixel(x,(Y-y),color);
      c = c+w;
    }
  }
